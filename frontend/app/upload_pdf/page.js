...
export default function Upload() {
  ...
  useEffect(() => {
    setAvailableURL(
      `${window.location.protocol}//${window.location.host}/company/${companyName}/${productName}`
    );
  ...
  const onSubmit = async (data) => {
    ...
    if (!data.file[0]) console.log("No file selected");
      ...
      const add_to_db = await fetch(
        `${process.env["NEXT_PUBLIC_BACKEND_URL"]}/add_manual_to_db?company_name=${companyName}&product_name=${productName}&product_device=${productType}&file_name=${data.file[0].name}`,
        {
          method: "POST",
        }
      ).then((data) => data.json());
      ...
      const uploadRes = await fetch(
        `${process.env["NEXT_PUBLIC_BACKEND_URL"]}/upload?manual_id=${add_to_db.manual_id}&manual_name=${productName}`,
        {
          method: "POST",
          body: formData,
        }
      ...
      <div className="flex mt-10 w-[700] min-w-[400] justify-center items-center">
        <div className="flex flex-col p-10 border-2  border-gray-400 dark:border-gray-600 rounded-md shadow-md shadow-gray-300 dark:shadow-slate-900 gap-4">
          ...
          >
            ...
            <article>
              <label htmlFor="manualname" className="font-medium mb-2">
                Product Name
              </label>
              <input
                disabled={isLoading}
                id="manualname"
                className="w-full rounded-md border-0 bg-white/5 px-3.5 py-2 text-slate-900 font-sm shadow-sm ring-1 ring-inset dark:text-white  ring-gray/10 focus:ring-1 focus:ring-inset sm:text-sm sm:leading-6 hover:ring-gray-600"
            ...
            <article>
              <label htmlFor="companyname" className="font-medium mb-2">
                Company Name
              </label>
              <input
                disabled={isLoading}
                id="companyname"
                className="w-full rounded-md border-0 bg-white/5 px-3.5 py-2 text-slate-900 dart:text-white font-sm shadow-sm ring-1 ring-inset ring-gray/10 focus:ring-1 focus:ring-inset sm:text-sm sm:leading-6 hover:ring-gray-600"
            ...
            <article>
              <label htmlFor="select" className="font-medium mb-2">
                Product Type
              </label>
              <select
                disabled={isLoading}
              ...
              >
                ...
                <option value="oil diffuser">Oil Diffuser</option>
                <option value="air purifier">Air Purifier</option>
                <option value="vacuum cleaner">Vacuum Cleaner</option>
                <option value="fan">Coffee Maker</option>
              </select>
              <p className="error">{errors.productType?.message}</p>
            </article>
            <Button disabled={isLoading} id="upload" className={buttonStyle}>
                  ...
                  <div className="flex items-center gap-2 mr-4">
                    ...
                    <div className="flex flex-col">
                      <p className="text-slate-800 dark:text-slate-300">
                        Your knowledgebase is ready at:
                      </p>
                      <a className="hover:underline" href={availableURL}>
                        {availableURL}
                      </a>
                    </div>
                  </div>
